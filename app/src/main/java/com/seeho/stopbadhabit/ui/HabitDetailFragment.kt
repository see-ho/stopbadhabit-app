...
class HabitDetailFragment : Fragment() {
    ...
    private fun observeFailBottomSheetTrigger() {
        viewLifecycleOwner.lifecycleScope.launchWhenStarted {
            habitDetailViewModel.openDiaryBottomSheet.collect {
                Log.e("TAG", "observeFailBottomSheetTrigger: $it", )
                if(it){
                    mainViewModel.detailHabitId.value?.let { it1 ->
                        DiaryWriteFragment ({
                            mainViewModel.detailHabitId.value?.let { it1 ->
                            ...
                            Log.e("TAG", "observeFailBottomSheetTrigger: Triggered!", )
                        }, it1).apply {
                            show(this@HabitDetailFragment.childFragmentManager, "diaryWrite")
                        }
                    }
        ...
        mainViewModel.detailHabitId.observe(viewLifecycleOwner) {
            if (it != -1)
                ...
                Snackbar.make(
                    ...
                    String.format(requireActivity().resources.getString(R.string.habit_id_error)),
